...
Clients.extend({
  ...
  _client_list_index: function(credential_str) {
    for( var index in this.client_list ) {
      if( this.client_list[index].credential_str == credential_str ) {
        return index;
      }
    }
    return false;
  ...
  channel_queue: {
    ...
    add: function(credential_str, channel_str) {
      channel_list = this.channel_queue_dict[credential_str] ? this.channel_queue_dict[credential_str] : [];
